...
export default function CheckInSuccessPage() {
  ...
  return (
    <ScreenShell bottomSpacing aria-label="체크인 상태 확인 필요">
      <TopNavigation href={meetupPath} title="체크인 상태 확인" />
      <section className="px-[var(--dimension-x5)] pb-8 pt-12" aria-labelledby="check-in-contract-gap-heading">
        <h2 id="check-in-contract-gap-heading" className="m-0 font-display text-[length:var(--type-headline)] font-normal leading-8 text-[var(--fg-neutral)]">
          체크인 결과를 확인할 수 없어요
        </h2>
        <p className="m-0 mt-4 text-[length:var(--type-body)] leading-[22px] text-[var(--fg-muted)]">
          체크인 성공은 실제 체크인 요청의 201 응답에서만 표시해요. 이 주소만으로는 성공 여부를 복원할 수 없어요.
        </p>
      </section>
      <BottomActionBar>
        <Link

...
export default function JoinResultPage() {
  ...
  return (
    <ScreenShell bottomSpacing aria-label="참여 상태 확인 필요">
      <TopNavigation href={meetupPath} title="참여 상태 확인" />
      <section className="px-[var(--dimension-x5)] pb-8 pt-12" aria-labelledby="join-contract-gap-heading">
        <h2 id="join-contract-gap-heading" className="m-0 font-display text-[length:var(--type-headline)] font-normal leading-8 text-[var(--fg-neutral)]">
          참여 결과를 확인할 수 없어요
        </h2>
        <p className="m-0 mt-4 text-[length:var(--type-body)] leading-[22px] text-[var(--fg-muted)]">
          이 주소만으로는 실제 참여 요청의 결과를 복원할 수 없어요. 모임 상세에서 최신 참여 상태를 확인해 주세요.
        </p>
      </section>
      <BottomActionBar>
        <Link

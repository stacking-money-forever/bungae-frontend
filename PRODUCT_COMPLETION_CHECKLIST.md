...
- [x] Backend의 기존 `/api/v1` 전 계약면을 live SSOT `/v1`에 맞추고 controller, security, WebSocket, OpenAPI, inventory, traceability, validation, tests를 함께 갱신했다.
  ...
  - `POST /v1/me/verification-sessions`를 Notion의 `본문 없음` 계약에 맞췄다. Client `returnUrl`을 제거하고 server-owned HTTPS `VERIFICATION_RETURN_URL`만 사용하며 누락·비HTTPS·credential·fragment 설정은 503으로 fail-closed, 요청 본문은 400으로 거절한다. JDK 21의 profile 단위·contract·security·integration test와 configuration-cache 저장·재사용을 독립 확인했다.
...
- [ ] Frontend API client를 Notion `/v1` 계약에 연결한다.
  - Auth/Push 범위의 typed client와 `application/problem+json` 처리는 완료했다. Access/refresh token은 메모리 전용이며 concurrent refresh와 계정 전환이 session identity로 격리된다. 나머지 meetup 흐름 연결이 남았다.
...
- [ ] 가입/OTP/검증/프로필/로그아웃 흐름이 실제 서버 상태로 동작한다.
  ...
  - [x] 인증 세션의 `/v1/me` GET/PATCH, `If-Match` 충돌 복구, `/v1/activity-policies` 기반 관심 활동, bodyless `/v1/me/verification-sessions`와 HTTPS provider handoff UI를 연결했다. 계정 전환 시 이전 검증 URL·편집 draft·저장 pending/error가 새 subject로 넘어가지 않는 회귀 테스트가 통과한다.
...
- [ ] 탐색/생성/참여/대기/확정/취소/채팅/체크인/사후 흐름이 reload와 다른 세션에서도 유지된다.
  - [x] 모임 상세가 fresh `allowedActions`만으로 `JOIN`, `LEAVE`, `CANCEL`, `QUORUM_DECISION`, `CHECK_IN`을 노출한다. JOIN/LEAVE는 실제 API mutation, in-flight 중복 차단, 성공·action error 후 상세 재조회, 동일 JOIN 재시도 key, route/account stale completion 격리를 검증했다. 취소·정족수 결정·체크인은 서버가 허용한 경우에만 기존 실제 route로 연결된다.
...
- [ ] 정확한 장소, 채팅, 신고자·증거가 응답·HTML·캐시·로그에서 권한에 맞게 마스킹된다.
  ...
  - 나머지 chat·신고·증거 projection의 DTO·DB·runtime privacy scanner와 party/owner/reporter 정책은 통과했다. 실제 frontend HTML/browser 표면 검증은 남았다. 안전한 대략 위치의 허용 정밀도·표현은 `PRODUCT_DECISION_REQUIRED`라 현재 아무 위치도 반환하지 않는다.

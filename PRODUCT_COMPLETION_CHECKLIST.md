...
- [ ] 탐색/생성/참여/대기/확정/취소/채팅/체크인/사후 흐름이 reload와 다른 세션에서도 유지된다.
  ...
  - 2026-09-21 실제 QA backend(`https://bungae-qa.justn.me`, 격리 DB) + Chromium 390×844에서 QA-1 OTP 로그인 → 원래 모임 상세 복귀 → 실제 서버 데이터 렌더와 relation 기반 action(참여 취소만 노출, JOIN 없음)을 확인했다. QA 환경은 장소 provider 미설정(`PLACE_PROVIDER_UNAVAILABLE`)이고 시드 모임 1건의 시작 시각이 지나 생성·정족수·체크인 수명주기는 실행할 수 없었다. access/refresh token이 메모리 전용이라 새로고침 후에는 익명 게이트로 돌아간다.
